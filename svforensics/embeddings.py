"""
Speaker embedding extraction using ECAPA2.

This module provides functions to extract speaker embeddings from audio files
using the ECAPA2 speaker embedding model. The embeddings can be used for
speaker verification and other speaker recognition tasks.
"""
...
logger = logging.getLogger("svforensics.embeddings")
...
class EmbeddingExtractor:
    """
    Speaker embedding extractor using ECAPA2 model.
    
    This class provides methods to extract speaker embeddings from audio files.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract speaker embeddings from audio files")
    
    # Processing mode
    subparsers = parser.add_subparsers(dest="mode", help="Processing mode")
    
    # Single directory mode
    single_parser = subparsers.add_parser("single", help="Process a single directory")
    single_parser.add_argument(
        "input_dir",
        help="Input directory containing audio files or speaker subdirectories"
    )
    single_parser.add_argument(
        "--output-file",
    ...
    pr_parser = subparsers.add_parser("probe-ref", help="Process probe and reference directories")
    ...
    print(f"SVforensics Speaker Embedding Extraction Utility")
    print(f"===============================================")
    ...
    try:
        ...
        if args.mode == "single":
            ...
            # Print summary
            num_probe_speakers = len(probe_embeddings)
            num_ref_speakers = len(reference_embeddings)
            ...
            print(f"\nProcessing complete!")
            print(f"Probe embeddings: {num_probe_speakers} speakers, {total_probe_files} files")
            print(f"Reference embeddings: {num_ref_speakers} speakers, {total_ref_files} files")
